...
def step(dot: Dot, path: Path, resolution: Tuple[int, int], dt: float) -> Dot:
    dot.position = dot.position + dot.velocity * dt
...
def main():
    ...
    resolution = (400, 400)
    sdf = compute_sdf(path, (400, 400), resolution)
    dt = 0.025
    for t in np.arange(0, 10, dt):
        # step
        dots = [step(dot, path, resolution, dt) for dot in dots]
